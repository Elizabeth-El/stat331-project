...
This plot illustrates the relationship between GDP per capita (on a log scale) and life expectancy across different regions. Each point represents a country within a given region. A red linear regression line is fitted to the overall data, showing a positive association as countries with higher GDP per capita tend to have longer life expectancies. This is faceted by region to identify the varying strengths of this relationship across the globe. As we can see, South America has the largest life expectancy growth rate for increasing log gdp. Oceania on the other hand, has the slowest life expectancy growth rate.
```{r}
#| code-fold: true
