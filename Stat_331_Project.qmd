...
To determine how much of the variability in Life Expectancy is accounted for by the regression model, we can calculate the proportion by dividing the Variance in Fitted Values by the Variance in Response: `r var_fitted` / `r var_response` ≈ `r (var_fitted / var_response)` This value is validated in the Linear Regression Fit Information table, as this number matches the R squared value. This tells us that `r (var_fitted / var_response) * 100`% of the variability in life expectancy is explained by the model, indicating a moderate-to-strong relationship between GDP per capita and life expectancy. The remaining `r (1 - (var_fitted / var_response)) * 100`% of the variability (Variance in Residuals) is unexplained by the model, suggesting that there are other extraneous factors that also influence life expectancy.
